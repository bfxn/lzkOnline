...
from lzkOnline.settings import EMAIL_FROM
...
def send_register_email(email,send_type="register"):
    ...
    if send_type == "register":
        email_title = "在线教育网欢迎您。"
        email_body = "终于等到你，请点击下面的链接激活你的账号：http://127.0.0.1:8000/active/{0}".format(code)
    ...
    if send_type == "forget":
        ...
        # 使用django内置函数完成邮件发送。四个参数：主题，邮件内容，从哪里发，接受者（list）
        send_status = send_mail(email_title,email_body,EMAIL_FROM,[email])
        # 如果发送成功
        if send_status:
            pass
